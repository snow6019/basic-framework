 ...
 */
package com.lxzh.basic.framework.sys.modular.auth.context;
...
import com.lxzh.basic.framework.core.context.login.LoginContext;
...
public class LoginContextSpringSecurityImpl implements LoginContext {
    ...
    public static LoginContextSpringSecurityImpl me() {
        return SpringUtil.getBean(LoginContextSpringSecurityImpl.class);
    }
    ...
    public boolean hasPermission(String requestUri) {
        if (this.getSysLoginUser() != null && (!Objects.nonNull(this.getSysLoginUser().getLoginPortType()) || !Objects.equals(this.getSysLoginUser().getLoginPortType(), LoginPortTypeEnum.NOT_RECTIFY.getCode()))) {
            return true;
        }
        ...
        return matchPermission(this.getSysLoginUser().getPermissions(), requestPermission);
    }

    @Override
    public boolean hasPermission(String requestUri, String method) {
        SysLoginUser userInfo = this.getSysLoginUser();
        if (this.getSysLoginUser() != null && (!Objects.nonNull(this.getSysLoginUser().getLoginPortType()) || !Objects.equals(this.getSysLoginUser().getLoginPortType(), LoginPortTypeEnum.NOT_RECTIFY.getCode()))) {
            return true;
        }
        boolean flag = false;
        ...
        if (ObjectUtil.isNotEmpty(permissionList)) {
            ...
            if (StrUtil.isNotBlank(method)) {
                flag = matchPermission(permissionList, method + StrUtil.SPACE + requestPermission);
            }
            if (!flag) {
                flag = matchPermission(permissionList, requestPermission);
            }
        }
        return flag;
    }

    public boolean matchPermission(List<String> permissions, String requestPermission) {
        for (String temp : permissions) {
            boolean flag = antPathMatcher.match(temp, requestPermission);
            if (flag) {
                return true;
            }
        }
