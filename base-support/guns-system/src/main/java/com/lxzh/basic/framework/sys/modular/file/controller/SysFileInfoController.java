 ...
 */
package com.lxzh.basic.framework.sys.modular.file.controller;
...
public class SysFileInfoController {
    ...
    @ApiOperation(value = "文件信息表_上传文件", notes = "文件信息表_上传文件")
    @BusinessLog(title = "文件信息表_上传文件", opType = LogAnnotionOpTypeEnum.OTHER)
    public DCResponse<UploadFileVO> upload(@RequestPart("file") MultipartFile file) {
        SysFileInfo sysFileInfo = sysFileInfoService.uploadFile(file);
        return DCResponse.success(new UploadFileVO(sysFileInfo));
    }
